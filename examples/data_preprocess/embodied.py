...
def dmake_prefix(dp, template_type):
    target = dp['target']
    numbers = dp['nums']
...
def make_prefix(dp, template_type):
    ...
    if template_type == 'base':
        """This works for any base model"""
        prefix = f"""        Consider that you are embodied in the following agent and environment. 
        ...
        Show your work in <think> </think> tags. And return the final as a set of conditional rules in python and put them in <answer> </answer> tags, for example 
        <answer>     if pole_velocity > 1.1:
        return 1
    if cart_velocity < -1.1:
        return 0
    if cart_angle >= 1.0:
        return 1
    else:
...
<think>"""
        ...
        Agent: A cart over a track that has an unactuated pole attached to it. You can move the cart right or left in order to keep the pole balanced.
    ...
    Show your work in <think> </think> tags. And return the final answer as if else conditional rules in <answer> </answer> tags, for example <answer> if cart_velocity < -1.1:
        go left
    else go right
    </answer>.<|im_end|>\n<|im_start|>assistant\nLet me solve this step by step.\n<think>"""
    return prefix
